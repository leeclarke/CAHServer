...
public class GeneralExceptionMapper implements ExceptionMapper<Exception>{
	...
	public Response toResponse(Exception exception) {
		...
		if(exception instanceof NullPointerException){
		...
		} else if(exception instanceof CAHAppException){
			return new CAHAppExceptionMapper().toResponse((CAHAppException)exception);
			...
			return Response
				.status(status)
                .entity(new CAHAppException(status, msg, cause,0).toResponse())
		...
		return Response
            ...
            .entity(new CAHAppException(Status.BAD_REQUEST, msg, "",0).toResponse())
            .type(MediaType.APPLICATION_JSON)
            .build();
	}
